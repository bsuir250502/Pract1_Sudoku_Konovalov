...
int analysis_rowsNcolumns(cell_t ** table)
{
    ...
    if (!table[a_][b_].num) {
        for (j = 0; j < 9; j++)
            if (table[a_][j].num)
                table[a_][b_].poss[table[a_][j].num - 1] =
                    table[a_][j].num;
        for (i = 0; i < 9; i++)
            if (table[i][b_].num)
                table[a_][b_].poss[table[i][b_].num - 1] =
                    table[i][b_].num;
    }
    return 1;
}
